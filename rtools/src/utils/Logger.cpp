...
namespace Ruby
{
    ...
    Logger::LoggerPtr<LoggerTraits::VendorLogger> Logger::MakeLog(void) const
    {
        if (!m_logger)
            std::fprintf(stderr, "Logger::MakeLog() : logger cannot be empty: You must first call Logger::Init() before making log!");
